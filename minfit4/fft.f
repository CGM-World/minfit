...
c..
      implicit none
      integer            i,n,m,isign,nmax,no2
      parameter          (nmax = 16384)
      double precision   data(n),respns(n)
      double complex     fft(nmax),ans(n)
c..
c..
      do 11 i=1,(m-1)/2
...
c..
      ...
      do 13 i=1,no2+1
       if (isign.eq.1) then
        ...
        if (abs(ans(i)).eq.0.0) stop ' DEATH: in routine convlv (fft.f)'
         ans(i) = fft(i)/ans(i)/no2
       else
        stop ' DEATH: sign prob in routine convlv (fft.f)'
       endif
 13   continue
...
c..
      ...
      double precision   wr,wi,wpr,wpi,wtemp,theta,wrs,wis,
